...
bedtools intersect -a nhek-active.bed -b nhlf-active.bed > activeinboth.bed
wc -l activeinboth.bed 
#12174 activeinboth.bed
...
#These numbers add up to 14579 which is does not add up to the first command which is 14013
#To make an adjustment to make the numbers add up you, need to use -u 
bedtools intersect -u -a nhek-active.bed -b nhlf-active.bed > activeinboth.bed
wc -l activeinboth.bed 
#above commands reports 11608
bedtools intersect -v -a nhek-active.bed -b nhlf-active.bed > activeinnhek.bed
wc -l activeinboth.bed 
#2405 activeinnhek.bed
...
bedtools intersect -f 1 -a nhek-active.bed -b nhlf-active.bed > command1.bed
head command1.bed
#chr1	25558413	25559413

#-F 1, 100% in -b must overlap -a so NHEK active that contain A NHLF region 
bedtools intersect -F 1 -a nhek-active.bed -b nhlf-active.bed > command2.bed
head command2.bed
...
bedtools intersect -f 1 -F 1 -a nhek-active.bed -b nhlf-active.bed > command3.bed
head command3.bed
...
bedtools intersect -a nhek-active.bed -b nhlf-active.bed > bothactive.bed   
head bothactive.bed
#chr1	19923013	19924213

#active in NHEK repressed in NHLF
bedtools intersect -a nhek-active.bed -b nhlf-repressed.bed > 1EKActive.bed
head 1EKActive.bed
#chr1	1981140	1981540

#repressed in NHEK and NHLF 
bedtools intersect -a nhek-repressed.bed -b nhlf-repressed.bed > bothrepressed.bed
