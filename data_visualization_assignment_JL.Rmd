...
```{r q1}
View(dt)
breaks_seq = breaks = seq(0, 100, 2)
ggplot (dt, aes ( x = Liver)) +
  geom_histogram(breaks = breaks_seq, color = 'black', fill = 'pink') + 
  scale_x_continuous(limits = c(0,100), breaks = seq(0,100,2)) +
  labs (
    x = "Liver",
    y = "Frequency of Genes",
    title = "Gene Expression in Liver (Histogram)"
  ) + theme_classic()

breaks_seq = breaks = seq(0, 100, 2)
ggplot (dt, aes ( x = Ovary)) +
  geom_histogram(breaks = breaks_seq, color = 'black', fill = 'pink') + 
  scale_x_continuous(limits = c(0,100), breaks = seq(0,100,2)) +
  labs (
    x = "Ovary",
    y = "Frequency of Genes",
    title = "Gene Expression in Ovary (Histogram)"
  ) + theme_classic()

# answer here
#The distribution follows a positive skew pattern. This tells me that the relative expression of genes in the liver transcriptome are expressed at a low level. There are only a few genes showing higher levels of expression in the Liver. I repeated the same graph with the Ovary and saw the same pattern. Therefore, it does not seem like it's Liver tissue specific. 
...
d2p <- ggplot (dt, aes ( x = Liver + 1)) +
  geom_density(color = 'black', fill = 'pink') +
  scale_x_log10()+
  labs (
    x = "Liver (Log 10 TPM + 1)",
    y = "Density",
    title = "Gene Expression in Liver"
  ) + theme_classic()
...
ggplot (dt, aes ( x = Liver + 1, y =`Heart - Left Ventricle` + 1)) +
  geom_point(color = 'pink', fill = 'pink', size = 1, alpha = 0.2) + 
  geom_abline(linetype = "dashed", intercept = 0, slope = 1, color = 'red') +
  scale_x_log10()+
  scale_y_log10() + 
  labs (
    x = "Expression Genes in Liver (Log 10 TPM + 1)",
    y = "Expression Genes in Heart - Left Ventricle (Log 10 TPM + 1)",
    title = "Gene Expression in Liver" 
  ) + theme_classic()
```
...
# answer here
dt_exp <- dt %>%
  mutate(is_mitochondrial = str_starts(Description,"MT-"))  %>%  group_by(is_mitochondrial)

plot <- 
ggplot (dt_exp, aes ( x = Liver + 1, y =`Heart - Left Ventricle` + 1, color = is_mitochondrial)) +
  geom_point( size = 1, alpha = 0.2) + 
  geom_abline(linetype = "dashed", intercept = 0, slope = 1, color = 'pink') +
  scale_x_log10()+
  scale_y_log10() + 
  labs (
    x = "Expression Genes in Liver (Log 10 TPM + 1)",
    y = "Expression Genes in Heart - Left Ventricle (Log 10 TPM + 1)",
    title = "Gene Expression in Liver") + theme_classic()
...
cincoplot <- ggplot (dt_long , aes ( x = TPM)) +
  geom_histogram(breaks = breaks_seq, color = 'pink', fill = 'pink') + 
  scale_x_continuous(limits = c(0,100))+
  facet_grid(.~Tissue, scales = "free_y") +
  labs (
    x = "Tissues",
    y = "Frequency of Genes",
    title = "Gene Expression in Focal Tissues (Histogram)"
  ) + theme_classic()
cincoplot
...
tenplot <- ggplot (dt_long , aes ( x = TPM)) +
  geom_histogram(breaks = breaks_seq, color = 'pink', fill = 'pink') + 
  scale_x_continuous(limits = c(0,100))+
